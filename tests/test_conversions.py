...
def test_compact_axis_angle():
    ...
    for _ in range(5):
        a = pr.random_axis_angle(rng)
        ca = libtrans.axang_to_compact_axang(a)
        assert_array_almost_equal(libtrans.normalize_vec(ca), a[:3])
        assert pytest.approx(np.linalg.norm(ca)) == a[3]
...
def test_quaternion_hamilton():
    ...
    q_ij = libtrans.quat_mul(pr.q_i, pr.q_j)
    assert_array_equal(pr.q_k, q_ij)
    q_ijk = libtrans.quat_mul(q_ij, pr.q_k)
...
def test_quaternion_conjugate():
    ...
    for _ in range(5):
        ...
        v = pr.random_vector(rng)
        vq = libtrans.quat_prod_vec(q, v)
        vq2 = libtrans.quat_mul(
            libtrans.quat_mul(q, np.hstack(([0], v))),
            libtrans.quat_conj(q)
        )[1:]
...
def test_quaternion_invert():
    ...
    q_inv = libtrans.quat_conj(q)
    q_q_inv = libtrans.quat_mul(q, q_inv)
...
def test_quaternion_rotation_consistent_with_multiplication():
    ...
    for _ in range(5):
        ...
        qv_mult = libtrans.quat_mul(
            q, libtrans.quat_mul(v_im, libtrans.quat_conj(q)))[1:]
